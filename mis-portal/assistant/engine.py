...
#
# web_search is deliberately pinned to the BASIC 20250305 variant, not 20260209. The
# newer one does "dynamic filtering": it runs code execution internally to trim search
# results. That internal sandbox issues a *programmatic tool call* into our 14 client-side
# tools, and the follow-up request then requires the `container_id` of the sandbox that
# made it — which the response carrying the pending call reports as None, so there is
# nothing to send back. Every such turn dies on:
...
# We never asked for programmatic tool calling (no tool sets `allowed_callers`), so the
# fix is to stop using the variant that triggers it. Verified 2026-07-28 across three
# multi-turn prompts: 20260209 fails with or without CODE_EXECUTION_TOOL alongside it;
# 20250305 + code_execution completes. The cost is dynamic filtering's token trimming on
# search results; general-purpose code execution and all 14 tools are unaffected.
